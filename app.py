...
from src.llm import summarize_recommendations, _get_client, transcribe_audio
...
# ---------------------------------------------
# Session state setup (place this BEFORE st.text_area)
# ---------------------------------------------
if "query_text" not in st.session_state:
    st.session_state["query_text"] = ""

# If a new transcript was just created, move it into query_text
if "pending_transcript" in st.session_state:
    st.session_state["query_text"] = st.session_state["pending_transcript"]
...
# ---------------------------------------------
# Optional: Voice input using Whisper
# ---------------------------------------------
st.subheader("🎙 Or say it out loud")

audio_file = st.file_uploader(
    "Upload a short audio clip (MP3, WAV, M4A, OGG):",
...
if st.button("Transcribe voice to text"):
    if audio_file is None:
        ...
        if transcript.startswith("(transcription failed"):
            ...
            st.warning("Transcription was empty. Try speaking a bit longer or more clearly.")
        else:
            st.success("Transcription complete! You can edit it above before searching.")
            # Store transcript in a temporary key
            st.session_state["pending_transcript"] = transcript
            # Trigger a rerun; on the next run we’ll copy this into query_text *before* the text_area
            st.rerun()
