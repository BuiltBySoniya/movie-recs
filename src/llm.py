...
import os
import io  # NEW: for in-memory audio files
...
def transcribe_audio(
    ...
    """
    Use OpenAI Whisper to turn an uploaded audio file into text.

    Parameters
    ----------
    file_bytes : bytes
        Raw bytes from the uploaded audio file (Streamlit upload).
    filename : str
        Name of the file (for logging/metadata).
    model : str
    ...
    -------
    str
        Transcribed text, or an error message starting with '(transcription failed'.
    """
    client = _get_client()

    # Wrap bytes in a file-like object that OpenAI can consume
    audio_file = io.BytesIO(file_bytes)
    audio_file.name = filename  # some clients like having a name
    ...
    try:
        ...
        # New OpenAI client: transcription text is in resp.text
